...
function doGet(e) {
  ...
  if (action === 'syncAll') {
    ...
    if (videoLastRow > 1) {
      sheet.getRange(2, 8, videoLastRow - 1, 12).clearContent();
    }
    if (data.videos && data.videos.length > 0) {
      var videoRows = [];
      for (var j = 0; j < data.videos.length; j++) {
        var v = data.videos[j];
        videoRows.push([v.id, v.datum, v.kunde, v.projekt, v.videoart, v.anzahl, v.geliefert || 0, v.stueckpreis || 0, v.betrag || 0, v.status, v.quelle || 'Freelance', v.notiz]);
      }
      sheet.getRange(2, 8, videoRows.length, 12).setValues(videoRows);
    }
